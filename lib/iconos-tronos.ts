...
export function colorAntifaz(h: Hermandad): string {
  ...
  if (a.includes("morado")) return "#4A154B";
...
/**
 * Ilustración vectorial de un nazareno malagueño en miniatura:
...
function svgNazareno(x: number, tunica: string, antifaz: string, escala = 1): string {
  ...
  <g transform="translate(${x} 0) scale(${escala})">
    <path d="M0 4 L-5 20 L5 20 Z" fill="${antifaz}" stroke="#D4AF37" stroke-width="0.8"/>
    <circle cx="0" cy="21" r="4.2" fill="${antifaz}" stroke="#D4AF37" stroke-width="0.8"/>
    <circle cx="1.6" cy="20.4" r="1" fill="#1A1A1A"/>
    <path d="M-6 26 Q0 23.5 6 26 L7.5 46 L-7.5 46 Z" fill="${tunica}" stroke="#D4AF37" stroke-width="0.8"/>
    <rect x="-6.6" y="34" width="13.2" height="2.4" fill="#D4AF37"/>
    <rect x="7" y="24" width="1.8" height="16" rx="0.9" fill="#F5F1E8" stroke="#A8842B" stroke-width="0.5"/>
    <rect x="6.6" y="21.6" width="2.6" height="3" rx="1" fill="#D4AF37"/>
...
/**
 * Miniatura de trono malagueño con varales, palio (Virgen) o cruz (Cristo),
...
function svgTronoMini(tipo: "cristo" | "virgen", tunica: string, antifaz: string): string {
  ...
  const remate =
    tipo === "virgen"
      ? `<path d="M30 6 L32 11 L37 11 L33.2 14 L34.7 19 L30 16 L25.3 19 L26.8 14 L23 11 L28 11 Z" fill="#D4AF37"/>`
      : `<rect x="28.6" y="5" width="2.8" height="13" rx="1" fill="#D4AF37"/><rect x="24" y="9" width="12" height="2.8" rx="1" fill="#D4AF37"/>`;
  const palio =
    tipo === "virgen"
      ? `<rect x="14" y="20" width="32" height="5" rx="1.5" fill="#4A154B" stroke="#D4AF37" stroke-width="1"/>
         <rect x="15.5" y="25" width="2" height="9" fill="#D4AF37"/>
         <rect x="42.5" y="25" width="2" height="9" fill="#D4AF37"/>`
      : `<rect x="16" y="24" width="28" height="3.4" rx="1.2" fill="#4A154B" stroke="#D4AF37" stroke-width="0.9"/>`;
  return `
  <g>
    ${nazarenos}
    <rect x="17" y="41" width="26" height="4.5" rx="1.5" fill="#A8842B" stroke="#D4AF37" stroke-width="0.8"/>
    <rect x="19" y="45.5" width="3" height="3" fill="#8A6D1F"/>
    <rect x="38" y="45.5" width="3" height="3" fill="#8A6D1F"/>
    <rect x="19.5" y="27" width="1.6" height="14" fill="#D4AF37"/>
    <rect x="39" y="27" width="1.6" height="14" fill="#D4AF37"/>
    <circle cx="30" cy="34" r="4" fill="${tipo === "virgen" ? "#4A154B" : "#2b1436"}" stroke="#D4AF37" stroke-width="0.9"/>
    ${palio}
    ${remate}
  </g>`;
}
 ...
 *  - halo dorado `#D4AF37` pulsante SOLO sobre el trono cuyo GPS emite en vivo
